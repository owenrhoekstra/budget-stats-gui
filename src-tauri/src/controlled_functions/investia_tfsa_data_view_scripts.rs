...
#[tauri::command]
pub fn investia_tfsa_data_run_built_in_script(script_name: String) -> Result<String, String> {
    match script_name.as_str() {
        "gen_stats_investia_tfsa" => {
            let input = "investia_tfsa".to_string();
            let output = gen_stats_investia_tfsa::investia_tfsa_balance_averages(input);
            println!("{}", output);
            Ok(output)
        }
...
#[tauri::command]
pub async fn investia_tfsa_data_run_imported_script(pool: tauri::State<'_, ReadPool>, handler: String, db_path_state: tauri::State<'_, std::path::PathBuf>) -> Result<String, String> {
    ...
    let mut imported_script_payload_json = serde_json::json! ({
        ...
        "caller": "investia_tfsa_data_view",
        "expected_db_path_extension": "db/budget-stats-gui.db",
        "expected_table_name": "investia_tfsa",
...
#[tauri::command]
pub fn investia_tfsa_data_get_built_in_scripts() -> Vec<Script> {
    vec![
        gen_stats_investia_tfsa::get_script_info(),
    ]
}
...
#[tauri::command]
pub async fn investia_tfsa_data_get_imported_scripts(pool: tauri::State<'_, ReadPool>) -> Result<Vec<Script>, String> {
