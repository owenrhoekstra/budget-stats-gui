...
async function runPackagedScript(scriptName: string) {
  ...
  try {
    scriptOutputs.value[scriptName] = await invoke("tangerine_chequing_data_run_built_in_script", { scriptName });
  } catch (err) {
...
async function runImportedScript(handler: string) {
  ...
  try {
    ...
    scriptOutputs.value[handler] = await invoke("tangerine_chequing_data_run_imported_script", { handler });
  } catch (err) {
...
onMounted(async () => {
  try {
    packagedScripts.value = await invoke(
        "tangerine_chequing_data_get_built_in_scripts",
    );
  } catch (err) {
  ...
  try {
    importedScripts.value = await invoke(
        "tangerine_chequing_data_get_imported_scripts",
    );
  } catch (err) {
